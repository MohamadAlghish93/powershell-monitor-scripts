  ...
  Import-Csv "Servers.csv" | foreach { 
    ...
    $Current_Server_IP | ForEach-Object -Process {
      ...
      Invoke-Command -Session $session   -ArgumentList $Current_Server_IP -ScriptBlock {
        ...
        $DirectoryLogsTxt = "C:\$Current_Server_IP"
        If (!(test-path -PathType container $DirectoryLogsTxt)) {
          New-Item -ItemType Directory -Path $DirectoryLogsTxt
        }
        $LogsDir = "$DirectoryLogsTxt\PerfNVSLogs.txt" 
